...
@section('page_badge', 'ویرایش روش ارسال')
@section('breadcrumb')
...
@section('content')
<form method="POST" action="{{ route('admin.shipping-methods.update', $shippingMethod) }}" class="max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
@csrf @method('PUT')

<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1.5">عنوان فارسی <span class="text-red-500">*</span></label>
        <input type="text" name="title_fa" value="{{ old('title_fa', $shippingMethod->title_fa) }}" required
    ...
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $shippingMethod->title) }}" required dir="ltr"
...
<div>
    ...
    <textarea name="description" rows="2"
              class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-brandBlue resize-none">{{ old('description', $shippingMethod->description) }}</textarea>
</div>

<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1.5">هزینه ارسال (تومان) <span class="text-red-500">*</span></label>
        <input type="number" name="base_cost" value="{{ old('base_cost', $shippingMethod->base_cost) }}" required min="0"
    ...
    <div>
        ...
        <input type="text" name="city" value="{{ old('city', $shippingMethod->city) }}"
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-5">
    <div>
        ...
        <input type="number" name="min_days" value="{{ old('min_days', $shippingMethod->min_days) }}" required min="0"
    ...
    <div>
        ...
        <input type="number" name="max_days" value="{{ old('max_days', $shippingMethod->max_days) }}" required min="0"
    ...
    <div>
        ...
        <input type="number" name="sort_order" value="{{ old('sort_order', $shippingMethod->sort_order) }}" required min="0"
               class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-brandBlue">
    </div>
</div>
...
<div>
    ...
    <input type="text" name="estimated_delivery_text" value="{{ old('estimated_delivery_text', $shippingMethod->estimated_delivery_text) }}"
           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-brandBlue"
...
<label class="flex items-center gap-3 cursor-pointer">
    ...
    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $shippingMethod->is_active) ? 'checked' : '' }} class="w-4 h-4 text-brandBlue rounded">
