...
@section('page_badge', 'کد تخفیف جدید')
@section('breadcrumb')
...
@section('content')
<form method="POST" action="{{ route('admin.coupons.store') }}" class="max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
@csrf
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div>
        ...
        <input type="text" name="code" value="{{ old('code') }}" required dir="ltr"
               ...
               placeholder="GAMEPEK20">
        <p class="text-xs text-gray-400 mt-1">فقط حروف لاتین، اعداد، خط تیره و زیرخط</p>
    ...
    <div>
        ...
        <select name="type" id="typeSelect" required
            ...
            <option value="percentage" {{ old('type', 'percentage') === 'percentage' ? 'selected' : '' }}>درصدی</option>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div>
        ...
        <input type="number" name="value" value="{{ old('value') }}" required min="1"
...
<div>
    ...
    <input type="number" name="min_order_amount" value="{{ old('min_order_amount') }}" min="0"
           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-brandBlue">
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div>
        ...
        <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" dir="ltr"
               class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brandBlue">
    </div>
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1.5">تاریخ انقضا (اختیاری)</label>
        <input type="datetime-local" name="ends_at" value="{{ old('ends_at') }}" dir="ltr"
...
(function() {
    ...
    function syncLabel() {
        valueLabel.innerHTML = typeSelect.value === 'percentage'
            ? 'مقدار تخفیف (٪) <span class="text-red-500">*</span>'
            : 'مقدار تخفیف (تومان) <span class="text-red-500">*</span>';
    }
