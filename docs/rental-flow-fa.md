...
> وضعیت تست‌ها (تأیید‌شده با اجرا در ۱۷ شهریور ۱۴۰۵): `OK (171 tests, 1140 assertions)`

> ⚠️ **این سند معماری «کدِ امروز» را توصیف می‌کند، نه سیاست تأییدشدهٔ
> کسب‌وکار را.** از ۱۷ شهریور ۱۴۰۵ مجموعه‌ای از تصمیم‌های کسب‌وکار تأیید شده
> که در چند نقطه با رفتار فعلی کد **تفاوت** دارد — مهم‌ترین‌ها:
>
> - رزرو فقط **پس از پرداختِ تأییدشده** انجام می‌شود (کد فعلی اول رزرو می‌کند)
> - **هیچ hold پرداخت‌نشده‌ای وجود ندارد** (کد فعلی دارد)
> - **KYC باید قبل از پرداخت** کامل شود (کد فعلی هیچ gate ای ندارد)
> - هر `Product` می‌تواند **چند Device Unit** با شماره سریال جدا داشته باشد
>
> فهرست کامل تصمیم‌های تأییدشده، شکاف‌های پیاده‌سازی، و مواردی که هنوز
> تصمیم‌گیری/بررسی حقوقی می‌خواهند:
> [`docs/business/CONFIRMED_DECISIONS.md`](business/CONFIRMED_DECISIONS.md)
>
> تا زمانی که آن شکاف‌ها بسته نشده‌اند، توضیحات زنجیرهٔ اجاره در این سند
> همچنان معتبرِ «وضعیت فعلی» است.
...
```bash
vendor/bin/phpunit            # OK (171 tests, 1140 assertions) — تأیید‌شده ۱۷ شهریور ۱۴۰۵
```
